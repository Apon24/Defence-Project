...
export const Challenges = () => {
  ...
  // Helper function to normalize challenge data (map _id to id)
  ...
  const loadChallenges = async () => {
    ...
    try {
      ...
      if (data.length > 0) {
        // Normalize challenges to map _id to id
        const normalized = data.map(normalizeChallenge);
        ...
        const normalized = (newResponse.data || []).map(normalizeChallenge);
        setChallenges(normalized);
      }
    } catch (error) {
